...
function ClassicClockTowerDial({ decimalGhati, isLive, data, panchang, tzH, simple, wallTime }: {
  ...
  return (
    <div className="relative select-none" style={{ width: "100%", aspectRatio: "1" }}>
      ...
      {/* Hover Tooltip for Classic Theme */}
      {hoveredGraha && (
        <div style={{
          position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
          background: 'rgba(6, 5, 10, 0.95)', border: '1px solid rgba(212, 175, 55, 0.4)',
          borderRadius: '12px', padding: '16px', boxShadow: '0 8px 32px rgba(0,0,0,0.8)',
          pointerEvents: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', zIndex: 50,
          minWidth: '160px', backdropFilter: 'blur(4px)'
        }}>
          <div style={{ color: hoveredGraha.config.color, fontSize: '32px', fontWeight: 'bold', lineHeight: 1 }}>{hoveredGraha.config.symbol}</div>
          <div style={{ color: 'white', fontSize: '18px', fontFamily: "'Cinzel', serif", fontWeight: 'bold' }}>
            {hoveredGraha.config.sanskrit} <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px', fontFamily: "'Outfit', sans-serif" }}>({hoveredGraha.name})</span>
          </div>
          <div style={{ height: '1px', width: '100%', background: 'rgba(212, 175, 55, 0.3)', margin: '4px 0' }}></div>
          <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', fontFamily: "monospace" }}>Lon: {hoveredGraha.longitude?.toFixed(2)}°</div>
          <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '12px', fontFamily: "monospace" }}>Lat: {hoveredGraha.latitude?.toFixed(2) || '0.00'}°</div>
          {hoveredGraha.speed < 0 && <div style={{ color: '#f87171', fontSize: '12px', fontWeight: 'bold', marginTop: '4px', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Retrograde</div>}
        </div>
      )}
    </div>
  );
...
function PremiumClockTowerDial({ decimalGhati, isLive, data, panchang, tzH, simple, wallTime }: {
  decimalGhati: number;
  isLive: boolean;
  data: any;
  panchang: any;
  ...
  return (
    <div className="relative select-none" style={{ width: "100%", aspectRatio: "1" }}>
        ...
        {grahas.map((p: any) => {
          ...
          return (
            ...
            >
              ...
              <text x={gx} y={gy + 28} textAnchor="middle" dominantBaseline="central" fill={isHovered ? "white" : "rgba(255,255,255,0.6)"} fontSize="16" fontFamily="'Outfit', sans-serif" fontWeight="bold" style={{ ...legibilityStyle, pointerEvents: "none" }}>
                {p.config.sanskrit}
              </text>
            </g>
          );
      ...
      {/* Hover Tooltip for Premium Theme */}
      {hoveredGraha && (
        <div style={{
          position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
          background: 'rgba(6, 5, 10, 0.95)', border: '1px solid rgba(212, 175, 55, 0.4)',
...
export default function Home() {
  ...
  const fetchData = useCallback(async (targetISO: string) => {
    ...
    try {
      ...
      if (json.success) { setData(json.data); setMuhurat(json.muhurat); setTimeline(json.timeline); setPanchang(json.panchang); }
  ...
  const vt = (() => {
    if (!data) return { ghati: 0, pala: 0, vipala: 0, decimalGhati: 0, simple: { hours: 0, minutes: 0, seconds: 0 } };
    ...
    return {
      ...
      decimalGhati: norm,
      simple: { hours: simpleHours, minutes: simpleMinutes, seconds: simpleSeconds },
    };
  })();
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#06050a", color: "#fbf9f4",
      ...
      {/* ── Top bar ─────────────────────────────────────────────────────── */}
      <div style={{
        width: "100%", maxWidth: "800px", display: "flex", justifyContent: "space-between",
        alignItems: "center", marginBottom: "12px"
      }}>
        <span style={{
          fontFamily: "'Cinzel',serif", fontSize: "13px", color: "#d4af37",
      ...
      {showSettingsPanel && (
        <div style={{
          width: "100%", maxWidth: "480px", background: "rgba(13,11,22,0.9)",
          ...
          {[["Lat", "number", lat, setLat], ["Lon", "number", lon, setLon], ["Alt (m)", "number", alt, setAlt]].map(([lbl, , val, setter]: any) => (
            <label key={lbl} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              ...
              <input type="number" value={val} onChange={e => setter(parseFloat(e.target.value) || 0)}
                style={{
                  background: "rgba(0,0,0,0.5)", border: "1px solid rgba(138,43,226,0.3)",
            ...
            <div style={{ display: "flex", gap: "6px" }}>
              {["Live", "Travel"].map(m => (
                <button key={m} onClick={() => setIsLive(m === "Live")}
                  style={{
                    flex: 1, padding: "4px", borderRadius: "6px", fontSize: "10px", fontWeight: 700,
      ...
      {activeView === "app" && (
        <>
          ...
          {data && (
            <div style={{
              width: "100%", maxWidth: "800px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr",
              gap: "8px", marginBottom: "12px"
            }}>
              {data.prominentMuhurats.map(mh => (
                <div key={mh.name}
                  style={{
                    background: mh.isActive
                  ...
                  <div style={{
                    fontSize: "8px", color: mh.type === "auspicious" ? "#d4af37" : "#f87171",
          ...
          <div style={{
            ...
            {data?.festival && (
              <button onClick={() => setShowFestivalModal(true)}
                style={{
                  background: `linear-gradient(90deg,${data.festival!.color}18,transparent)`,
                ...
                <div>
                  <span style={{
                    fontSize: "8px", color: data.festival!.color,
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "10px" }}>
                ...
                <div>
                  ...
                  <div style={{
                    ...
                    {fmt2(vt.simple?.hours ?? 0)} : {fmt2(vt.simple?.minutes ?? 0)} : {fmt2(vt.simple?.seconds ?? 0)}
                  </div>
                  <div style={{ fontSize: "10px", color: "rgba(212,175,55,0.7)", marginTop: "2px", letterSpacing: "0.08em" }}>
                    {fmt2(vt.ghati)}:{fmt2(vt.pala)}:{fmt2(vt.vipala)} (Ghati/Pala)
                  </div>
                </div>

                <div style={{ height: "1px", background: "rgba(212,175,55,0.12)" }} />

                {/* Sunrise & Muhurta Info */}
                <div style={{ display: "flex", gap: "10px", marginTop: "2px", marginBottom: "2px" }}>
                  <div style={{ background: "rgba(212,175,55,0.08)", padding: "6px 10px", borderRadius: "6px", flex: 1 }}>
                    <div style={{ fontSize: "8px", color: "rgba(212,175,55,0.6)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "2px" }}>Sunrise Today</div>
                    <div style={{ fontSize: "14px", color: "#d4af37", fontWeight: 700, fontFamily: "'Cinzel',serif" }}>{fmtTime(panchang?.sunriseUsed || data?.sunriseStart || '', tzH)}</div>
                  </div>
                  <div style={{ background: "rgba(212,175,55,0.08)", padding: "6px 10px", borderRadius: "6px", flex: 1 }}>
                    <div style={{ fontSize: "8px", color: "rgba(212,175,55,0.6)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "2px" }}>Current Muhurta</div>
                    <div style={{ fontSize: "14px", color: "#d4af37", fontWeight: 700, fontFamily: "'Cinzel',serif", display: "flex", alignItems: "center", gap: "6px" }}>
                      <span>{vt.simple?.hours ?? 0} / 30</span>
                      {panchang && (
                        <span style={{ fontSize: "10px", color: "rgba(212,175,55,0.7)" }}>({panchang.muhurta.sanskrit})</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <div style={{
                    fontSize: "9px", color: "rgba(138,43,226,0.7)",
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                <button onClick={() => setShowPlanetModal(true)}
                  style={{
                    fontSize: "9px", color: "#a78bfa", background: "none", border: "none",
                  ...
                  return (
                    <div key={p.name} style={{
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        {p.isRetrograde && (
                          <span style={{
                            fontSize: "7px", color: "#f87171", fontWeight: 700,
            ...
            {muhurat && (
              <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{
                    marginTop: "5px", height: "4px", borderRadius: "2px",
                    background: "rgba(255,255,255,0.06)", overflow: "hidden"
                  }}>
                    <div style={{
                      height: "100%", width: `${mScore}%`,
            ...
            {timeline.length > 0 && (
              <div>
                ...
                <div style={{
                  display: "flex", gap: "2px", height: "28px", alignItems: "flex-end",
                  ...
                  {timeline.map((h: any, i: number) => {
                    ...
                    return (
                      <div key={i} title={`${h.score}% — ${new Date(h.time).getUTCHours()}:00`}
                        style={{
                          flex: 1, background: c, borderRadius: "2px",
        ...
        <Modal title={data.festival.name} onClose={() => setShowFestivalModal(false)}>
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <div style={{
              ...
              <div style={{
                fontSize: "10px", color: data.festival.color,
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  {["Planet", "Sanskrit", "Longitude", "Sign", "°", "Status"].map(h => (
                    <th key={h} style={{
                      padding: "4px 6px", textAlign: "left", color: "#52525b",
              ...
              return (
                <button key={i} onClick={() => {
                  ...
                  <span style={{
                    ...
                    fontFamily: "monospace", width: "36px"
                  }}>{hourLabel}</span>
                  <div style={{
                    ...
                    <div style={{
                      height: "100%", width: `${h.score}%`, background: c,
                      borderRadius: "3px", transition: "width 0.5s"
                    }} />
                  </div>
                  <span style={{
                    fontSize: "12px", fontWeight: 700, color: c,
                    fontFamily: "'Cinzel',serif", width: "36px", textAlign: "right"
                  }}>{h.score}%</span>
